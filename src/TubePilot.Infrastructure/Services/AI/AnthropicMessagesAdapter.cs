...
{
    // Anthropic's Messages API wire format (api.anthropic.com and Anthropic-compatible
    // gateways). BaseUrl/Model/ApiKey all come from the caller's AiTool row.
    public class AnthropicMessagesAdapter : IAiFormatAdapter
    {
        private const string AnthropicVersion = "2023-06-01";
        private readonly HttpClient _http;

        public string ApiFormat => AiApiFormats.AnthropicMessages;

        public AnthropicMessagesAdapter(HttpClient http)
        {
            _http = http;
        }
        ...
        public async Task<AiResponse> GenerateAsync(string baseUrl, string apiKey, string model, AiRequest request, CancellationToken cancellationToken = default)
        {
            ...
            using var httpRequest = new HttpRequestMessage(HttpMethod.Post, CombineUrl(baseUrl, "v1/messages"));
